...
var MarshalRawsTests = []marshalTests{
	{
		line:     line(),
		input:    map[string]json.Marshaler{"slice of raw jsons": pfmt.Raws([]byte(`{"foo":{"bar":{"xyz":"Hello, Wörld!"}}}`), []byte("[42]"))},
	...
	{
		line:      line(),
		input:     map[string]json.Marshaler{"raws with quote": pfmt.Raws([]byte(`Hello, "Wörld"!`), []byte("[42]"))},
		want:      `Hello, "Wörld"! [42]`,
	...
	{
		line:     line(),
		input:    map[string]json.Marshaler{"quoted raws": pfmt.Raws([]byte(`"Hello, Wörld!"`), []byte("[42]"))},
		want:     `"Hello, Wörld!" [42]`,
		wantText: `"Hello, Wörld!" [42]`,
		wantJSON: `{
	...
	{
		line:      line(),
		input:     map[string]json.Marshaler{"raws with nested quote": pfmt.Raws([]byte(`"Hello, "Wörld"!"`), []byte("[42]"))},
		want:      `"Hello, "Wörld"!" [42]`,
		wantText:  `"Hello, "Wörld"!" [42]`,
	...
	{
		line:      line(),
		input:     map[string]json.Marshaler{"raw quoted jsons": pfmt.Raws([]byte(`"{"foo":"bar"}"`), []byte("[42]"))},
		want:      `"{"foo":"bar"}" [42]`,
		wantText:  `"{"foo":"bar"}" [42]`,
	...
	{
		line:      line(),
		input:     map[string]json.Marshaler{"slice of raw malformed json objects": pfmt.Raws([]byte(`xyz{"foo":"bar"}`), []byte("[42]"))},
		want:      `xyz{"foo":"bar"} [42]`,
	...
	{
		line:      line(),
		input:     map[string]json.Marshaler{"slice of raw malformed json key/value": pfmt.Raws([]byte(`{"foo":"bar""}`), []byte("[42]"))},
		want:      `{"foo":"bar""} [42]`,
		wantText:  `{"foo":"bar""} [42]`,
	...
	{
		line:      line(),
		input:     map[string]json.Marshaler{"slice of raw json with unescaped null byte": pfmt.Raws(append([]byte(`{"foo":"`), append([]byte{0}, []byte(`xyz"}`)...)...), []byte("[42]"))},
		want:      "{\"foo\":\"\u0000xyz\"} [42]",
		wantText:  "{\"foo\":\"\u0000xyz\"} [42]",
	...
	{
		line:      line(),
		input:     map[string]json.Marshaler{"slice of empty raws": pfmt.Raws([]byte{}, []byte{})},
		want:      " ",
		wantText:  " ",
	...
	{
		line:     line(),
		input:    map[string]json.Marshaler{"slice of raw nils": pfmt.Raws(nil, nil)},
		want:     "null null",
		wantText: "null null",
		wantJSON: `{
